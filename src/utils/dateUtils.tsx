export const getRelativeTime = (timestamp: string | number | Date): string => {
  ...
  //   const reelPattern2 = /^reels:\/\/share\/reel\/([a-f\d]{24})$/;
  //   const userPattern1 =
  //     /^http:\/\/localhost:3000\/share\/user\/([a-zA-Z0-9_]+)$/;
  //   const userPattern2 = /^reels:\/\/share\/user\/([a-zA-Z0-9_]+)$/;
  ...
  // handle both http(s) + custom scheme
  const reelPattern = /\/share\/reel\/([a-f\d]{24})$/;
  const userPattern = /\/share\/user\/([a-zA-Z0-9_]+)$/;

  if (reelPattern.test(url)) {
    type = 'reel';
    id = url.match(reelPattern)![1];
  } else if (userPattern.test(url)) {
    type = 'user';
    id = url.match(userPattern)![1];
  } else {
